...
class BoundingBoxTracker():
	...
	def computeDistanceMatrix(self, a: List[BoundingBox], b: List[BoundingBox]) -> np.ndarray:
		mA = len(a)
		mB = len(b)
		matrix = np.zeros((mA, mB))

		for i in range(mA):
			for j in range(mB):
				matrix[i,j] = 1.0 - a[i].IoU(b[j])
	...
	def update(self, rects: List[BoundingBox]) -> OrderedDict:
		...
		# centroids
			...
			# object centroid
			D = self.computeDistanceMatrix(objectBBoxes, rects)
